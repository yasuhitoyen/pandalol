...
const GameSummary = (match) => {
  ...
  return (
    <>
      <div
        className={`flex flex-row rounded-lg shadow-xl ${
          victory
            ? "bg-gradient-to-r from-lime-100 via-lime-50 to-lime-100"
            : "bg-gradient-to-r from-violet-200 via-violet-100 to-violet-200"
        } h-[100px] lg:w-[900px] w-[800px] transition-all duration-200 ease-in-out transform hover:scale-[1.01] border ${
      ...
      >
        ...
        <div className="flex flex-col px-2 py-2 w-[110px]  items-center justify-center">
          <h4 className="text-slate-700 w-full text-[15px] text-center ">
          ...
          <h4 className="w-24 w-fill text-center text-[15px] text-slate-700 ">
        ...
        <div className="flex flex-col justify-center space-y-2 w-[320px]  ">
          ...
          <div className="flex flex-row space-x-2 mt-2 justify-center w-full">
            {mainPlayer && (
              <Item
                item={`http://ddragon.leagueoflegends.com/cdn/${currentVersion}/img/item/${mainPlayer.item0}.png`}
              />
            )}
            {mainPlayer && (
              <Item
                item={`http://ddragon.leagueoflegends.com/cdn/${currentVersion}/img/item/${mainPlayer.item1}.png`}
              />
            )}
            {mainPlayer && (
              <Item
                item={`http://ddragon.leagueoflegends.com/cdn/${currentVersion}/img/item/${mainPlayer.item2}.png`}
              />
            )}
            {mainPlayer && (
              <Item
                item={`http://ddragon.leagueoflegends.com/cdn/${currentVersion}/img/item/${mainPlayer.item3}.png`}
              />
            )}
            {mainPlayer && (
              <Item
                item={`http://ddragon.leagueoflegends.com/cdn/${currentVersion}/img/item/${mainPlayer.item4}.png`}
              />
            )}
            {mainPlayer && (
              <Item
                item={`http://ddragon.leagueoflegends.com/cdn/${currentVersion}/img/item/${mainPlayer.item5}.png`}
              />
            )}
          </div>
          {/* End if Items Div */}
          <div className="flex flex-row justify-evenly items-center  w-full">
            ...
            <div className="flex justify-center align-middle">
              ...
              >
                <span className="">
                  {isFinite((Math.round(mainPlayerKDA * 100) / 100).toFixed(2)) ? (Math.round(mainPlayerKDA * 100) / 100).toFixed(2) : "Perfect"}
                </span>
              </div>
            </div>
            ...
            <div>
              ...
              ({mainPlayerFarmCount}) {(Math.round(mainPlayerFarmRate * 100) / 100).toFixed(1)}/min
              </span>
            </div>
          </div>
        </div>

        <div className="w-[250px] flex flex-row justify-evenly ml-auto" id="UsernameList">
          <div
            id="BlueSide"
            className="flex flex-col w-30 text-left justify-evenly h-full py-1 "
          >
            <SummonerNameDisplay
              player={players[0]}
              currentVersion={currentVersion}
